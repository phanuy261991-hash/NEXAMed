...
**Đơn giá dịch vụ khám (chốt 2026-08-26, `docs/DECISIONS.md` #079)**: mỗi dịch vụ khám (`reference_catalog` category `EXAM_TYPE`) có thể có nhiều dòng đơn giá theo Loại giá dịch vụ + Đơn vị tính + khoảng ngày hiệu lực, tách riêng theo tenant (bảng `exam_type_price`). Đây là mở rộng có giới hạn của v1 — **KHÔNG** phải "Price Book" đầy đủ.

**Sổ quỹ & Thu chi tại quầy (chốt 2026-09-05, `docs/DECISIONS.md` #121/#122/#124, kế hoạch kỹ thuật đầy đủ ở `reactive-moseying-flask.md`/`jiggly-meandering-leaf.md`)**: mở rộng tiếp của "Thu ngân cơ bản" — lý do: thu ngân chi/thu tiền mặt ngoài dịch vụ khám (tiền điện, tiền nước...) trước đây không có chỗ ghi, làm "Chốt ca" luôn lệch phải giải trình. Gồm: bảng `cash_account` (Quỹ tiền mặt/ngân hàng theo tenant, có số dư đầu kỳ) + `cash_voucher` (Phiếu thu/Phiếu chi ngoài dịch vụ khám, có thể yêu cầu duyệt) — **Giai đoạn 1 đã xong** (lập phiếu, in phiếu, tự cộng/trừ vào "Tiền mặt dự kiến trong két" của Chốt ca). **Giai đoạn 2 đã xong (2026-09-07)**: Sổ quỹ (liệt kê chứng từ theo quỹ, số dư luỹ kế) + Báo cáo dòng tiền (bảng kê thu/chi/tồn quỹ theo khoảng ngày, nhóm theo Loại thu chi/theo quỹ, xuất Excel — **KHÔNG** phải "báo cáo doanh thu theo kỳ" bị loại ở dòng dưới: không doanh thu, không công nợ, không lãi/lỗ) + tuỳ chọn "Thủ quỹ riêng" (mỗi thu ngân 1 két `DRAWER`) + phiếu "Chuyển quỹ" thủ công (di chuyển tiền giữa 2 quỹ nội bộ, không phải Thu/Chi thật) — **đã verify Playwright/trình duyệt thật (2026-09-07)**.

**Ví tạm ứng (chốt 2026-09-09, `docs/DECISIONS.md` #136)**: mở rộng tiếp của "Thu ngân cơ bản" — bệnh nhân nộp tiền trước (`patient_wallet`, 1 ví/bệnh nhân), hệ thống tự cấn trừ (`wallet_transaction`) lúc tiếp nhận nếu đủ số dư (invoice tự `PAID`, không qua quầy); thiếu thì thu ngân xử lý thủ công ở phiếu thu (trừ ví/nạp thêm/**trả hỗn hợp** — trừ hết ví + thu phần còn lại tiền mặt/CK trên cùng 1 phiếu, bật qua công tắc `walletMixedPaymentEnabled`, mặc định tắt). Nạp/tất toán ví tái dùng `cash_voucher` sẵn có (không bảng ledger quỹ riêng) — tự vào Chốt ca/Sổ quỹ/Báo cáo dòng tiền không cần sửa gì thêm. Huỷ lượt khám đã trừ ví → tiền trả ngược vào ví (không hoàn tiền mặt). Trang tổng hợp ví toàn phòng khám ở "Sổ quỹ & Thu chi" (`/cash-book/wallets`). **KHÔNG** theo đợt điều trị, **KHÔNG** phải "công nợ" hay "trả góp" (ngoài phạm vi v1, xem dòng dưới).

**Kho Thuốc & Vật tư y tế (chốt 2026-09-15, `docs/DECISIONS.md` #146, mở rộng phạm vi v1 — đảo ngược quyết định "dược/kho ngoài v1" của Sprint 4)**: mở rộng bảng `drug` sẵn có (không tạo bảng `items` mới), chuỗi quy đổi đơn vị N bậc (`drug_unit`), thẻ kho append-only làm nguồn sự thật (`stock_ledger`, GĐ2), nguyên tắc "đơn thuốc là y lệnh — chỉ Phiếu xuất kho mới sinh tiền/trừ kho" (1 đơn ↔ N phiếu xuất, GĐ3). Lộ trình 5 giai đoạn: **GĐ1 — Danh mục nền (đã xong 2026-09-15, `docs/DECISIONS.md` #148)**: mở rộng `drug` (loại mặt hàng thuốc/vật tư, quản lý theo lô, đơn vị cơ sở, giá bán mặc định, mã quốc gia...) + 4 bảng mới `drug_unit`/`drug_ingredient`/`supplier`/`warehouse` + 3 danh mục `reference_catalog` mới (Hoạt chất/Nhóm thuốc/Đường dùng), trang "Danh mục Thuốc & Vật tư" (`/admin/catalog-pharmacy`). **GĐ2 — Nhập kho & tồn theo lô (đã xong 2026-09-17, `docs/DECISIONS.md` #159)**: "Phiếu nhập kho" (luồng Nháp → Duyệt/Từ chối/Huỷ), 5 bảng mới `stock_receipt`/`stock_receipt_line`/`inventory_batch`/`stock_ledger`/`stock_balance`, giá vốn đích danh theo lô/bình quân gia quyền liên hoàn, "Tồn kho" + "Cảnh báo hạn dùng". **GĐ3 — Xuất kho theo đơn + FEFO + tiền thuốc (đã xong hoàn toàn, `docs/DECISIONS.md` #164/#165)**: "Phiếu xuất kho" luồng 1 bước, 2 bảng mới `stock_issue`/`stock_issue_line`, `invoice` thêm `invoice_type` (SERVICE/DRUG, duy nhất chạm bảng `invoice` đang chạy thật tại pilot), gợi ý lô FEFO, tự động phát thuốc lúc ký đơn (tuỳ chọn, gate qua `soloClinicWorkflowEnabled`), xem hoá đơn `DRUG` riêng qua `?invoiceId=`. **GĐ4 — Kiểm kê/Điều chuyển kho/Mở rộng Nhập-Xuất kho/Báo cáo Nhập-Xuất-Tồn — đã xong hoàn toàn (23/09/2026, `docs/DECISIONS.md` #170...#179)**: "Kiểm kê" (`stock_count`/`stock_count_line`, Nháp→Duyệt/Từ chối, đọc lại tồn SỐNG lúc Duyệt, tự sinh phiếu nhập/xuất bù chênh lệch); "Điều chuyển kho" (`stock_transfer`/`stock_transfer_line`, Duyệt xuất→Xác nhận nhận hàng, tự sinh cặp phiếu xuất/nhập liên kết); phân quyền theo Khoa/Phòng áp dụng đủ cho `stock_receipt`/`stock_issue`/`stock_count`/`stock_transfer`; "Phiếu xuất kho mở rộng" (3 loại Nháp→Duyệt lập tay: Xuất dùng nội bộ/Xuất trả NCC/Xuất huỷ) + "Phiếu nhập kho mở rộng" (`RETURN_FROM_USE` + Chiết khấu 2 mode Toàn phiếu/Từng dòng, chỉ áp `PURCHASE`) + "Báo cáo Nhập-Xuất-Tồn" (Đầu kỳ/Nhập/Xuất/Cuối kỳ theo mặt hàng, xuất Excel). **GĐ5 — Trải nghiệm kê đơn cho bác sĩ — chưa bắt đầu.**

**Công nợ nhà cung cấp (chốt 2026-09-24, `docs/DECISIONS.md` #180/#182, mở rộng phạm vi v1)**: mở rộng của Kho Thuốc GĐ2/GĐ3 — theo dõi công nợ phòng khám ↔ NCC phát sinh từ Phiếu nhập kho (`receiptType='PURCHASE'`). Module mới `supplier-debt`: `supplier_debt_account` (số dư snapshot/NCC) + `supplier_debt_entry` (sổ append-only, phân bổ FIFO tính lúc đọc, không lưu bảng phân bổ riêng) + `supplier_debt_adjustment` (Phiếu điều chỉnh/Đề nghị huỷ, Phần D, chưa tạo). Không đụng Sổ quỹ/Chốt ca/Báo cáo dòng tiền — tái dùng nguyên `cash_voucher` (thêm cột `supplier_id`), đúng tiền lệ Ví tạm ứng #136. Lộ trình 5 phần: **Phần A — Nền sổ công nợ (đã xong 2026-09-24, `docs/DECISIONS.md` #183)**: ghi PURCHASE lúc Duyệt phiếu nhập (đúng `netAmount` sau chiết khấu) + "Trả ngay" tuỳ chọn ngay trên phiếu nhập (sinh `cash_voucher` cùng lúc) + Khai nợ đầu kỳ (1 lần/NCC, số âm được — NCC nợ lại) + trang chi tiết NCC (`/suppliers/:id`). **Phần B — Thanh toán công nợ trên TỔNG nợ (đã xong 2026-09-24, `docs/DECISIONS.md` #184)**: trang "Công nợ nhà cung cấp" (`/suppliers/debts`) + "Phiếu thanh toán NCC" (`/suppliers/payments`) + nút/dialog "Thanh toán công nợ" trên trang chi tiết NCC (chặn trả vượt, cộng dồn cả phiếu đang chờ duyệt). **Phần C — Trả hàng NCC (code + test HTTP xong 2026-09-25, `docs/DECISIONS.md` #185, CHƯA verify Playwright)**: dùng lại "Phiếu xuất kho mở rộng" (GĐ4, `issueType='RETURN_TO_SUPPLIER'`) — gắn `supplierId`/`sourceReceiptId` (tuỳ chọn) + `stock_issue_line.return_unit_price` (giá trả riêng, không tái dùng `sell_price`); Duyệt ghi bút toán `RETURN` (trừ đúng phiếu nhập gốc trước nếu có chọn, không thì FIFO). "Thu tiền NCC hoàn lại" (`POST /supplier-debt/:supplierId/refund`, chỉ khi `balance<0`) sinh `cash_voucher` INCOME → `REFUND_RECEIVED`. **Huỷ phiếu xuất trả ĐÃ DUYỆT KHÔNG tự đảo công nợ** — cùng lỗ hổng đã biết với "Huỷ phiếu nhập" (`StockReceiptService.voidReceipt()`), cả hai để dành Phần D xử lý đồng bộ. **Phần D — Luồng xử lý sai sót (chưa bắt đầu)**: Huỷ chứng từ (còn đủ tồn, xử lý luôn 2 lỗ hổng trên)/Phiếu điều chỉnh công nợ (hàng đã dùng một phần)/Đề nghị huỷ (không có quyền duyệt). **Phần E — Đối chiếu & chốt công nợ theo kỳ (chưa có mockup, chưa bắt đầu)**: đúng khuôn "Khoá bảng ca" #110.

**Vẫn KHÔNG thuộc v1**: viện phí đầy đủ (bảng giá đa đối tượng theo nhiều loại khách hàng/bảo hiểm khác nhau cùng lúc), BHYT, báo cáo doanh thu theo kỳ (doanh thu/công nợ/lãi lỗ — khác bảng kê dòng tiền của Sổ quỹ GĐ2 ở trên, đã tách rõ ranh giới). **Công nợ/trả góp nhiều đợt theo lộ trình điều trị VẪN ngoài v1 — chỉ áp dụng cho công nợ BỆNH NHÂN** (khác "Công nợ nhà cung cấp" ở trên, đã vào v1 — hai khái niệm tách biệt, không nhầm lẫn). Không viết code cho các module ngoài phạm vi.
