...
**Version**: v1.56 — 24/09/2026 (xem mục 9 để biết lịch sử thay đổi)
...
### 3.10 Công nợ nhà cung cấp — Phần A "Nền sổ công nợ" + Phần B "Thanh toán" (`docs/DECISIONS.md` #180/#182/#183/#184)

**Đã hiện thực (v1.56, 24/09/2026)** — mở rộng tiếp của Kho Thuốc GĐ2/GĐ3 (mục 3.8), theo dõi công nợ phòng khám ↔ NCC phát sinh từ Phiếu nhập kho `receiptType='PURCHASE'` đã Duyệt. Migration `20260924100000_supplier_debt_phase_a` (viết tay, đã áp thật lên Postgres dev) — Phần B **không thêm bảng/cột nào**, tái dùng nguyên schema Phần A (`recordPayment()` chỉ tạo thêm dòng `cash_voucher`/`supplier_debt_entry` qua đúng cơ chế đã có). Lộ trình 5 phần đã chốt: **Phần A — Nền sổ công nợ (xong)** → **Phần B — Thanh toán trên TỔNG nợ (xong)** → Phần C — Trả hàng NCC (chưa xây) → Phần D — Luồng xử lý sai sót: Huỷ/Điều chỉnh/Đề nghị huỷ (chưa xây, bảng `supplier_debt_adjustment` **CHƯA tạo**) → Phần E — Đối chiếu & chốt công nợ theo kỳ (chưa có mockup).
...
5 permission mới `supplier_debt.read/pay/adjust/approve/unlock` — CHỈ `clinic_admin=global` lúc ra mắt (Q4 #182), vai trò khác chưa được cấp gì. Phần B tái dùng nguyên `supplier_debt.pay` (lập Thanh toán công nợ)/`.read` (2 endpoint mới `POST :supplierId/payment`/`GET payments`), không permission mới.

Trang web `/suppliers/:id` (chi tiết NCC — header + `StatCardRow` 3 số liệu + banner "chờ duyệt" + **3 tab** "Phiếu nhập"/"Sổ công nợ"/**"Thanh toán" (Phần B, mới)** + dialog "Khai nợ đầu kỳ" + **nút/dialog "Thanh toán công nợ" (Phần B, mới)**), `StockReceiptFormPage.tsx` thêm khối "Thanh toán" (`BoxedSection`, đúng mockup màn 5 `https://claude.ai/artifact/WvZtCgwbdEKAb9LcCzyCfh`) cạnh khối "Chiết khấu" có sẵn. `SupplierPane.tsx` thêm cột "Tổng mua"/"Còn nợ" + nút "Xem". **Phần B thêm 2 trang mới**: `/suppliers/debts` ("Công nợ nhà cung cấp" — bảng tổng hợp mọi NCC) và `/suppliers/payments` ("Phiếu thanh toán NCC" — lịch sử, lọc NCC/ngày/trạng thái) + badge số NCC chờ duyệt trên mục sidebar tương ứng (`shared/layout/Sidebar.tsx#NavItem` thêm prop `badge?: number` dùng chung).
