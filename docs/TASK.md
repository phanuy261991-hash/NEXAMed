...
- [ ] S1-06 — `packages/core`: khung entity, lớp lỗi + mã lỗi, `ports/` 6 interface, adapter no-op, đăng ký DI. Lớp lỗi (`DomainError` + 5 lỗi auth) và 1/6 port (`NotificationPort` + adapter no-op) đã có từ S1-04/S1-04c — còn 5 port (Storage/EventBus/Signature/Insurance/PatientIdentity) + khung entity
