...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <span className="mb-0.5 block text-xs font-semibold text-slate-800">{label}</span>
 ...
 */
export function PatientAdministrativeInfoCard({ patient }: { patient: PatientDetail }) {
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="flex items-center gap-2.5 border-b border-slate-100 px-5 py-3.5">
        <span className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-md bg-blue-600 text-white">
          <IdentificationCard size={16} weight="fill" aria-hidden="true" />
        </span>
        <h2 className="text-sm font-bold text-slate-800">Thông tin hành chính</h2>
      </div>

      <div className="grid grid-cols-2 gap-x-6 gap-y-4 p-5 sm:grid-cols-3 lg:grid-cols-5">
      ...
      {hasRelative && (
        <div className="px-5 pb-5">
          <div className="mb-2 text-[11px] font-bold uppercase tracking-wide text-slate-700">Thông tin người thân</div>
