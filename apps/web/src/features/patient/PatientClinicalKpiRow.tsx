import { ClipboardText, Warning as WarningIcon, Heartbeat, CalendarBlank } from '@phosphor-icons/react';
...
function BmiSparkline({ vitalSigns }: { vitalSigns: PatientVitalSignHistoryItem[] }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <svg width={SPARKLINE_WIDTH} height={SPARKLINE_HEIGHT} viewBox={`0 0 ${SPARKLINE_WIDTH} ${SPARKLINE_HEIGHT}`} aria-hidden="true">
      ...
      <span className="text-xs font-medium text-slate-500">BMI {values.length} lượt gần nhất — {trendLabel}</span>
    </div>
  );
}
...
export function PatientClinicalKpiRow({
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center justify-between gap-x-7 gap-y-3.5 rounded-[10px] border border-slate-200 bg-amber-50 px-6 py-4 shadow-sm">
        <div>
          <div className="mb-2 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-amber-800">
            <Heartbeat size={14} weight="bold" aria-hidden="true" />
            Sinh hiệu gần nhất
          </div>
          {latestBmi !== null && bmiClass ? (
            <div className="flex items-end gap-2.5">
              <div className={`text-[34px] font-extrabold leading-none tabular-nums ${bmiTextClass}`}>{latestBmi.toFixed(1)}</div>
              <span
      ...
      <div className="flex flex-wrap divide-x divide-slate-200 rounded-[10px] border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-[1_1_180px] flex-col items-center gap-1.5 px-[18px] py-3.5">
          ...
          <span className="text-[21px] font-extrabold tabular-nums text-slate-900">{totalCompletedVisits}</span>
        </div>
        <div className="flex flex-[1_1_180px] flex-col items-center gap-1.5 px-[18px] py-3.5">
          <span className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-slate-500">
          ...
          <span className="whitespace-nowrap text-[21px] font-extrabold tabular-nums text-slate-900">
        ...
        <div className="flex flex-[1_1_180px] flex-col items-center gap-1.5 px-[18px] py-3.5">
          ...
          <span className={`text-[21px] font-extrabold tabular-nums ${allergenCount > 0 ? 'text-rose-600' : 'text-slate-900'}`}>{allergenCount}</span>
        </div>
      </div>
    </div>
  );
