                  ...
                  <tbody>
                    {rooms.map((room) => (
                      <tr key={room.id} className={`border-b border-slate-200 last:border-0 ${room.isActive ? '' : 'opacity-50'}`}>
                        ...
                        <td className="px-4 py-2 text-center">
                          <StatusBadge tone={room.isActive ? 'success' : 'neutral'}>{room.isActive ? 'Đang hoạt động' : 'Ngưng dùng'}</StatusBadge>
                        </td>
                        {canManage && (
                          <td className="px-4 py-2 text-center">
                            <RowActionButton icon={PencilSimple} label="Sửa" tone="primary" onClick={() => setRoomModal({ mode: 'edit', room })} />
                          </td>
                        )}
                      </tr>
                    ))}
