...
const GRID_COLUMNS = '170px 1.4fr 190px 170px 150px 130px 120px';
...
export function InvoiceListPage() {
  ...
  return (
    <div className="flex h-full flex-col gap-2.5 p-3">
                ...
                {items.map((item) => (
                  ...
                  >
                    ...
                    <div role="cell" className="text-center">
                      {item.status === 'PAID' ? (
                        <span className="inline-flex items-center rounded-full bg-emerald-50 px-2.5 py-0.5 text-xs font-bold text-emerald-700">Đã thu</span>
                      ) : (
      ...
      {listQuery.isSuccess && (
        <div className="flex flex-shrink-0 flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-4.5 py-3">
          <div className="flex flex-col">
            <span className="text-[11px] font-bold uppercase tracking-wide text-slate-500">Tổng đã thu {formatDateLabel(date)}</span>
            <span className="text-xl font-extrabold tabular-nums text-brand-teal">{formatVnd(listQuery.data.paidTotalAmount)}</span>
          </div>
