...
describe('HTTP e2e — /api/v1/appointments', () => {
  ...
  describe('POST /api/v1/appointments/:id/checkin — check-in (docs/DECISIONS.md #032)', () => {
    it('receptionist check-in lịch SCHEDULED → 200, status CONVERTED', async () => {
      const created = await request(app.getHttpServer())
        .post('/api/v1/appointments')
        .set(authed(receptionistToken))
        .send(bookingPayload(doctorAUserId, isoAt(16, 0, 23)));
      const id = created.body.data.id as string;

      const res = await request(app.getHttpServer()).post(`/api/v1/appointments/${id}/checkin`).set(authed(receptionistToken)).send({ version: 1 });
    ...
    it('check-in lịch đã CANCELLED → 409 APPOINTMENT_NOT_CANCELLABLE', async () => {
      const created = await request(app.getHttpServer())
        .post('/api/v1/appointments')
        .set(authed(receptionistToken))
        .send(bookingPayload(doctorAUserId, isoAt(16, 30, 23)));
      const id = created.body.data.id as string;
      await request(app.getHttpServer())
        .post(`/api/v1/appointments/${id}/cancel`)
        .set(authed(receptionistToken))
      ...
      const res = await request(app.getHttpServer()).post(`/api/v1/appointments/${id}/checkin`).set(authed(receptionistToken)).send({ version: 2 });

      expect(res.status).toBe(409);
      expect(res.body.error.code).toBe('APPOINTMENT_NOT_CANCELLABLE');
    });

    it('check-in đã check-in rồi (đã CONVERTED) → 409 APPOINTMENT_NOT_CANCELLABLE', async () => {
      const created = await request(app.getHttpServer())
        .post('/api/v1/appointments')
        .set(authed(receptionistToken))
        .send(bookingPayload(doctorAUserId, isoAt(17, 0, 23)));
      const id = created.body.data.id as string;
      await request(app.getHttpServer()).post(`/api/v1/appointments/${id}/checkin`).set(authed(receptionistToken)).send({ version: 1 });
    ...
    it('version cũ → 409 CONCURRENT_MODIFICATION', async () => {
      const created = await request(app.getHttpServer())
        .post('/api/v1/appointments')
        .set(authed(receptionistToken))
        .send(bookingPayload(doctorAUserId, isoAt(17, 30, 23)));
      const id = created.body.data.id as string;

      const res = await request(app.getHttpServer()).post(`/api/v1/appointments/${id}/checkin`).set(authed(receptionistToken)).send({ version: 999 });

      expect(res.status).toBe(409);
      expect(res.body.error.code).toBe('CONCURRENT_MODIFICATION');
    });

    it('vai trò không có appointment.update (nurse) → 403 PERMISSION_DENIED', async () => {
      const created = await request(app.getHttpServer())
        .post('/api/v1/appointments')
        .set(authed(receptionistToken))
        .send(bookingPayload(doctorAUserId, isoAt(18, 0, 23)));
      const id = created.body.data.id as string;

      const res = await request(app.getHttpServer()).post(`/api/v1/appointments/${id}/checkin`).set(authed(nurseToken)).send({ version: 1 });

      expect(res.status).toBe(403);
    });

    it('tenant B không check-in được lịch hẹn của tenant A → 404', async () => {
      const created = await request(app.getHttpServer())
        .post('/api/v1/appointments')
        .set(authed(receptionistToken))
        .send(bookingPayload(doctorAUserId, isoAt(18, 30, 23)));
      const id = created.body.data.id as string;

      const res = await request(app.getHttpServer()).post(`/api/v1/appointments/${id}/checkin`).set(authed(tenantBReceptionistToken)).send({ version: 1 });
