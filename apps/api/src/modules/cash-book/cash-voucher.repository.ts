...
export interface CreateCashVoucherData {
  ...
  /** "Công nợ nhà cung cấp" (docs/DECISIONS.md #180/#182) — có giá trị = phiếu gắn 1 NCC. */
...
export class CashVoucherRepository {
   ...
   * trạng thái này. */
  async sumPendingApprovalBySupplierIds(tx: Prisma.TransactionClient, tenantId: string, supplierIds: string[]): Promise<Map<string, bigint>> {
    const map = new Map<string, bigint>();
    if (supplierIds.length === 0) return map;
    const rows = await tx.cashVoucher.groupBy({
      by: ['supplierId'],
      where: { tenantId, deletedAt: null, status: 'PENDING_APPROVAL', direction: 'EXPENSE', supplierId: { in: supplierIds } },
      _sum: { amount: true },
    });
    for (const row of rows) {
      if (row.supplierId) map.set(row.supplierId, row._sum.amount ?? 0n);
    }
    return map;
  }
