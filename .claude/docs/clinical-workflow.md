...
- `clinical_note` theo 6 mục nhóm "Thăm khám" (đổi từ 4 mục SOAP, `docs/DECISIONS.md` #060). Ký ghi chú (`signed_at`) đóng băng nội dung. "Tiền sử bản thân"/"gia đình"/"dị ứng" KHÔNG thuộc `clinical_note` — đọc/ghi thẳng `patient.personal_history`/`family_history`/`allergy_note` (dùng chung mọi lượt khám của bệnh nhân, không nhập lại mỗi lần khám, `docs/DECISIONS.md` #068).
