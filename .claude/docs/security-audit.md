...
- **Quy tắc trên áp dụng NGUYÊN VẸN cho `apps/web`, không chỉ backend** (bổ sung 2026-09-04 sau sự cố thật — xem `docs/DECISIONS.md` #119): TUYỆT ĐỐI không dùng `user.roles.includes('clinic_admin')`/`user.roles.some((r) => [...].includes(r))` để ẩn/hiện menu, nút, hay chặn route. Actor có thể giữ nhiều vai trò và quyền được gộp theo `maxDataScope` — kiểm tra PHẢI đi qua quyền thật (`useHasPermission`/`useDataScope`/`useHasAnyPermission`, `apps/web/src/features/auth/usePermission.ts`) đọc từ `currentUserSchema.permissions` (nguồn: `findAllPermissionsForUser()`, tính lúc login/`GET /auth/me`, khớp tuyệt đối logic `PermissionGuard`). Mọi route nghiệp vụ trong `router.tsx` phải bọc `RequirePermissionRoute`/`RequireAnyPermissionRoute` (`apps/web/src/features/auth/RequirePermissionRoute.tsx`) — ẩn menu không đủ, actor gõ thẳng URL vẫn phải bị chặn ở tầng route, không chỉ chờ từng API call trả 403 rải rác.
  - **Ngoại lệ CÓ CHỦ Ý, không phải sót**: gate theo TÊN VAI TRÒ vẫn hợp lệ khi đó là quyết định WORKFLOW đã chốt với chủ dự án (không phải proxy cho một quyền cụ thể) — ví dụ "Hàng đợi khám" chỉ dành cho bác sĩ dù điều dưỡng/lễ tân cũng có `encounter.read`. Các trường hợp này phải khai ở nguồn DUY NHẤT (`apps/web/src/features/auth/workflow-roles.ts`), dùng chung cho cả `Sidebar.tsx` lẫn route guard, kèm comment nêu rõ đây là quyết định nghiệp vụ. Lọc DANH SÁCH TÀI KHOẢN KHÁC theo `roleNames` (ví dụ dropdown chọn thu ngân) không thuộc quy tắc này — đó là lọc dữ liệu hiển thị, không phải chặn quyền của actor hiện tại.
- **Mọi hàm tra `role_permission`/`user_role` để quyết định quyền BẮT BUỘC lọc `deletedAt: null` ở CẢ 3 tầng**: dòng `role_permission`, dòng `user_role` lồng bên trong, và `role` (có thể bị ẩn qua "Vai trò & Phân quyền"). Thiếu 1 trong 3 khiến quyền đã thu hồi/vai trò đã gỡ vẫn còn hiệu lực vĩnh viễn — bug thật phát hiện 2026-09-04 ở `findScopesForUserPermission()` (nguồn tra quyền DUY NHẤT của `PermissionGuard`, ảnh hưởng MỌI endpoint được bảo vệ trong toàn hệ thống), xem test hồi quy `apps/api/src/infrastructure/persistence/permission-lookup.helper.spec.ts` và `docs/DECISIONS.md` #119. Viết hàm tra quyền mới (nếu thật sự cần, ưu tiên tái dùng `findScopesForUserPermission`/`findAllPermissionsForUser` có sẵn) phải có test hồi quy dạng "cấp quyền → xác nhận có; thu hồi qua soft-delete → xác nhận KHÔNG còn" — không chỉ test "cấp quyền thành công".
